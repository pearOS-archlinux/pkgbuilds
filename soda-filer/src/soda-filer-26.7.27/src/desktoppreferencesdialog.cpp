...
void DesktopPreferencesDialog::onBrowseClicked() {
  ...
  if(dlg.exec() == QDialog::Accepted) {
    ...
    // Picking an image only has any visible effect in a mode that actually
    // draws wallpaperFile_ (Stretch/Fit/Center/Tile) -- updateWallpaper()
    // ignores it entirely in WallpaperNone/WallpaperTransparent. Browsing
    // for a file while still on the default "Fill with background color
    // only" mode picked a file that was then silently never shown (desktop
    // stayed the flat background color), which looked like wallpaper
    // couldn't be set at all. Switch to a sensible default automatically
    // so choosing a file always shows it.
    int mode = ui.wallpaperMode->itemData(ui.wallpaperMode->currentIndex()).toInt();
    if(mode == DesktopWindow::WallpaperNone || mode == DesktopWindow::WallpaperTransparent) {
      int fitIndex = ui.wallpaperMode->findData(DesktopWindow::WallpaperFit);
      if(fitIndex >= 0)
        ui.wallpaperMode->setCurrentIndex(fitIndex); // triggers onWallpaperModeChanged -> applySettings()
      else
        applySettings();
    }
    else
