...
IconTheme::IconTheme():
  ...
  // QEvent::StyleChange doesn't reliably fire on every desktop/session type
  // when only the icon theme (not the widget style) is switched, which left
  // already-open windows -- most noticeably the Desktop window, which is
  // never closed/reopened -- stuck on the old theme's icons. Watch the KDE
  // config file directly as a more robust fallback trigger.
  QString kdeglobals = QStandardPaths::writableLocation(QStandardPaths::GenericConfigLocation) + QStringLiteral("/kdeglobals");
  if(QFileInfo::exists(kdeglobals)) {
    configWatcher_.addPath(kdeglobals);
    connect(&configWatcher_, &QFileSystemWatcher::fileChanged, this, &IconTheme::onConfigFileChanged);
  }
}

void IconTheme::onConfigFileChanged(const QString& path) {
  // Some editors/tools replace the file rather than write in place, which
  // drops it from the watch list, so re-add it if it still exists.
  if(QFileInfo::exists(path) && !configWatcher_.files().contains(path)) {
    configWatcher_.addPath(path);
  }
  // Delay slightly: our watcher can fire before KDE's own platform theme
  // plugin has finished re-reading the config and updating QIcon::themeName()
  // / QPalette::Highlight, so checking/consuming immediately can see stale
  // values.
  QTimer::singleShot(300, this, &IconTheme::checkChanged);
  QTimer::singleShot(300, this, &IconTheme::configFileChanged);
}
...
QIcon IconTheme::convertFromGIcon(GIcon* gicon) {
  if(G_IS_THEMED_ICON(gicon)) {
    ...
    // unwrap the emblemed icon and resolve its underlying base icon instead
    // of falling back to the generic "unknown" icon.
    GIcon* base = g_emblemed_icon_get_icon(G_EMBLEMED_ICON(gicon));
    return convertFromGIcon(base);
  }
