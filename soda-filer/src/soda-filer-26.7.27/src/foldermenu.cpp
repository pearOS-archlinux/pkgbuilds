...
FolderMenu::FolderMenu(FolderView* view, QWidget* parent):
  ...
  sortAction_ = new QAction(tr("Sort By"), this); // same wording as the desktop's context menu
  addAction(sortAction_);
  createSortMenu();
  sortAction_->setMenu(sortMenu_);

  // Clean Up / Clean Up By, same entries and animation as the desktop's
  // context menu (see DesktopWindow::updateMenu()/buildSortCriteriaMenu()).
  // Only meaningful for icon-mode views; FolderView::cleanUpIcons*() no-op
  // in list/compact modes.
  QAction* cleanUpAction = new QAction(tr("Clean Up"), this);
  addAction(cleanUpAction);
  connect(cleanUpAction, &QAction::triggered, view_, &FolderView::cleanUpIcons);

  QMenu* cleanUpByMenu = new QMenu(tr("Clean Up By"), this);
  struct CleanUpEntry { QString label; int column; };
  const CleanUpEntry cleanUpEntries[] = {
      { tr("Name"), FolderModel::ColumnFileName },
      { tr("Kind"), FolderModel::ColumnFileType },
      { tr("Date Modified"), FolderModel::ColumnFileMTime },
      { tr("Date Created"), FolderModel::ColumnFileCTime },
      { tr("Size"), FolderModel::ColumnFileSize },
      { tr("Tags"), FolderModel::ColumnFileTags },
  };
  for(const CleanUpEntry& e : cleanUpEntries) {
      QAction* action = cleanUpByMenu->addAction(e.label);
      int column = e.column;
      connect(action, &QAction::triggered, view_, [this, column]() { view_->cleanUpIconsBy(column); });
  }
  addAction(cleanUpByMenu->menuAction());
...
void FolderMenu::createSortMenu() {
  ...
  if(col >= 0 && col < FolderModel::NumOfColumns && sortActions_[col]) {
