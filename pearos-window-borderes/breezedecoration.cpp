...
namespace Breeze
{
    ...
    void Decoration::updateActiveState()
    {
        ...
        updateBorderOutline(); // its color follows the titlebar
        update();
    }
    ...
    QMarginsF Decoration::bordersFor(qreal scale) const
    {
        ...
        if (hideTitleBar())
            top = bottom;
        else if (m_internalSettings->macOSButtons())
        {
            // the pearOS titlebar has a fixed height
            top = KDecoration3::snapToPixelGrid(Metrics::TitleBar_HeightPx, scale);
        }
        else
        {
    ...
    void Decoration::recalculateBorders()
    {
        ...
        // the corners are kept round on all four sides, also when the window is
        // maximized or snapped against a screen edge; the top ones are only needed
        // when there is no titlebar to paint them
        qreal radius = 0;
        if (m_internalSettings->roundedCorners())
            radius = m_scaledCornerRadius;

        const qreal topRadius = hideTitleBar() ? radius : 0;
        setBorderRadius(KDecoration3::BorderRadius(topRadius, topRadius, radius, radius));

        updateBorderOutline();
    }

    //________________________________________________________________
    void Decoration::updateBorderOutline()
    {
        if (!m_internalSettings->macOSButtons()
            || !m_internalSettings->showOutline()
        ...
        // painting the outline ourselves only works over the titlebar, since the
        // decoration owns no pixels along the sides of a borderless window; the
        // compositor draws this one around the whole window, maximized included.
        // It sits against the desktop, so a translucent white would read as a hard
        // bright line; the titlebar color mixed with 20% white gives the same
        // result a translucent line painted over the titlebar used to give.
        const bool darkTitleBar(qGray(titleBarColor().rgb()) < 128);
        QColor outlineColor(KColorUtils::mix(titleBarColor(),
                                             darkTitleBar ? Qt::white : Qt::black,
                                             0.14));
        outlineColor.setAlpha(255);

        // half a pixel falls outside the window on the right and bottom edges, where
        // it gets clipped away, so the line keeps its full pixel and loses contrast
        // instead; the corners look thinner either way, that is the antialiasing
        setBorderOutline(KDecoration3::BorderOutline(1, outlineColor,
            KDecoration3::BorderRadius(m_scaledCornerRadius)));
    }
    ...
    //________________________________________________________________
    qreal Decoration::buttonGroupSideMargin() const
    {
        // titlebar padding around the button group; its trailing padding is
        // carried by the last button of the group instead
        if (m_internalSettings->macOSButtons()) return Metrics::MacOSTitleBar_Padding;
        return Metrics::TitleBar_SideMargin * settings()->smallSpacing();
    }

    //________________________________________________________________
    qreal Decoration::captionSideMargin() const
    {
        if (m_internalSettings->macOSButtons()) return Metrics::MacOSTitleBar_TitleGap;
        return Metrics::TitleBar_SideMargin * settings()->smallSpacing();
    }
    ...
    void Decoration::updateButtonsGeometry()
    {
        ...
        if (!m_leftButtons->buttons().isEmpty())
        {
            ...
            m_leftButtons->setSpacing(m_internalSettings->macOSButtons() ?
                Metrics::MacOSButton_Spacing : m_internalSettings->buttonSpacing());

            // padding
            const int vPadding = m_internalSettings->macOSButtons() ? 0
                : (isTopEdge() ? 0 : s->smallSpacing() * Metrics::TitleBar_TopMargin);
            const int hPadding = buttonGroupSideMargin();

            // trailing padding of the group
            if (m_internalSettings->macOSButtons())
            {
                auto button = static_cast<Button *>(m_leftButtons->buttons().back());
                QRectF geometry = button->geometry();
                geometry.adjust(0, 0, Metrics::MacOSButton_GroupRightPadding, 0);
                button->setGeometry(geometry);
                button->setRightPadding(Metrics::MacOSButton_GroupRightPadding);
            }
            if (isLeftEdge())
            {
        ...
        {
            // spacing (use our own spacing instead of s->smallSpacing()*Metrics::TitleBar_ButtonSpacing)
            m_rightButtons->setSpacing(m_internalSettings->macOSButtons() ?
                Metrics::MacOSButton_Spacing : m_internalSettings->buttonSpacing());
    ...
    void Decoration::paint(QPainter *painter, const QRectF &repaintRegion)
    {
        ...
        if (m_internalSettings->macOSButtons() && s->isAlphaChannelSupported())
        {
            painter->save();
            painter->setRenderHint(QPainter::Antialiasing);
            painter->setBrush(Qt::NoBrush);

            // the separator and the outline follow the titlebar brightness, so that
            // they stay visible on both dark and light color schemes
            const bool darkTitleBar(qGray(titleBarColor().rgb()) < 128);
    ...
    void Decoration::paintTitleBar(QPainter *painter, const QRectF &repaintRegion)
    {
        ...
        else
        {
            ...
            painter->drawRoundedRect(titleRect.adjusted(0, 0, 0, m_scaledCornerRadius),
    ...
    {
        // the pearOS artwork has a fixed size
        if (m_internalSettings->macOSButtons()) return Metrics::MacOSButton_Size;
    ...
    {
        const auto w = window();
        if (m_internalSettings->macOSButtons()) return borderTop();
    ...
    QPair<QRectF, Qt::Alignment> Decoration::captionRect() const
    {
        ...
        else
        {
            ...
            const auto w = window();
            const qreal sideMargin = captionSideMargin();
            const qreal leftOffset = KDecoration3::snapToPixelGrid(m_leftButtons->buttons().isEmpty() ?
                sideMargin + extraTitleMargin :
                m_leftButtons->geometry().x() + m_leftButtons->geometry().width() + sideMargin + extraTitleMargin,
                w->scale());

            const qreal rightOffset = KDecoration3::snapToPixelGrid(m_rightButtons->buttons().isEmpty() ?
                sideMargin + extraTitleMargin:
                size().width() - m_rightButtons->geometry().x() + sideMargin + extraTitleMargin,
                w->scale());

            const qreal yOffset = m_internalSettings->macOSButtons() ?
                0 : settings()->smallSpacing()*Metrics::TitleBar_TopMargin;
    ...
    void Decoration::updateShadow()
    {
            ...
            || g_shadowColor != m_internalSettings->shadowColor())
        {
            ...
            // the pearOS shadow is fixed: 0 0 1px rgba(0,0,0,0.8) + 0 18px 54px rgba(0,0,0,0.57)
            const CompositeShadowParams params = m_internalSettings->macOSButtons()
                ? CompositeShadowParams(QPoint(0, 18),
                                        ShadowParams(QPoint(0, 0), 54, 0.57),
                                        ShadowParams(QPoint(0, -18), 1, 0.8))
                : lookupShadowParams(g_shadowSizeEnum);
            if (params.isNone()) {
                g_sShadow.reset();
                g_sShadowInactive.reset();
                setShadow(shadow);
    ...
    void Decoration::setScaledCornerRadius()
    {
        ...
        m_scaledCornerRadius = KDecoration3::snapToPixelGrid(Metrics::Frame_FrameRadiusPx, window()->nextScale());
    }
