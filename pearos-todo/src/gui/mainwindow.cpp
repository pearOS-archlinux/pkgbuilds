...
void MainWindow::setupSidebar() {
    ...
    QLabel* listsLabel = new QLabel(tr("My Lists"), listsHeader);
    ...
    connect(m_sideList, &QListWidget::itemDoubleClicked, this, [this](QListWidgetItem* item) {
        ...
        if (row < 0 || row >= m_todoLists.size()) return;
        const QString currentName = m_todoLists.at(row).name;
        bool ok = false;
    ...
    connect(addListButton, &QPushButton::clicked, this, [this]() {
        TodoList nl;
        int idx = m_todoLists.size() + 1;
        nl.name = tr("New List %1").arg(idx);
        nl.id = nl.name.toLower().replace(' ', '-');
        m_todoLists.append(nl);
        m_currentListIndex = m_todoLists.size() - 1;
        saveTodos();
        refreshSidebarLists();
        if (m_currentListIndex >= 0 && m_currentListIndex < m_todoLists.size()) {
...
void MainWindow::setupContent() {
    ...
    m_contentLayout->addWidget(m_todoWidget, 1);

    loadTodos();
    refreshSidebarLists();

    connect(m_todoWidget, &TodoWidget::itemToggled, this, [this](int index, bool completed) {
        if (m_currentListIndex < 0 || m_currentListIndex >= m_todoLists.size()) return;
        TodoList& list = m_todoLists[m_currentListIndex];
        if (index < 0 || index >= list.items.size()) return;
        list.items[index].completed = completed;
        saveTodos();
    });

    connect(m_todoWidget, &TodoWidget::itemAdded, this, [this](const QString& text) {
        if (m_currentListIndex < 0 || m_currentListIndex >= m_todoLists.size()) return;
        TodoList& list = m_todoLists[m_currentListIndex];
        TodoItem item;
        item.text = text;
    ...
    connect(m_todoWidget, &TodoWidget::itemDeleted, this, [this](int index) {
        if (m_currentListIndex < 0 || m_currentListIndex >= m_todoLists.size()) return;
        TodoList& list = m_todoLists[m_currentListIndex];
        if (index < 0 || index >= list.items.size()) return;
    ...
    if (!m_todoLists.isEmpty()) {
        setCurrentList(0);
    }
}

void MainWindow::loadTodos() {
    m_todoLists = TodoStorage::load();
    if (m_todoLists.isEmpty()) {
        m_currentListIndex = -1;
    } else if (m_currentListIndex < 0 || m_currentListIndex >= m_todoLists.size()) {
        m_currentListIndex = 0;
    }
}

void MainWindow::saveTodos() const {
    if (m_todoLists.isEmpty()) return;
    if (!TodoStorage::save(m_todoLists)) {
        Logger::warning(QStringLiteral("Failed to save todo lists"));
    }
}
...
void MainWindow::refreshSidebarLists() {
    ...
    for (int i = 0; i < m_todoLists.size(); ++i) {
        const TodoList& list = m_todoLists.at(i);
        ...
        connect(deleteBtn, &QPushButton::clicked, this, [this, i]() {
            if (i < 0 || i >= m_todoLists.size()) return;
            ...
            if (m_todoLists.isEmpty()) {
                ...
                if (m_todoWidget) {
                    TodoList empty;
                    empty.name = tr("Empty");
                    m_todoWidget->setList(empty);
                }
            } else if (m_currentListIndex >= m_todoLists.size()) {
                m_currentListIndex = m_todoLists.size() - 1;
            }
            refreshSidebarLists();
            if (m_currentListIndex >= 0 && m_currentListIndex < m_todoLists.size()) {
    ...
    if (!m_todoLists.isEmpty()) {
        const int idx = (m_currentListIndex >= 0 && m_currentListIndex < m_todoLists.size())
                        ? m_currentListIndex
                        : 0;
...
void MainWindow::setCurrentList(int index) {
    ...
    if (m_todoWidget) {
        m_todoWidget->setList(m_todoLists.at(index));
    }
...
void MainWindow::createMenuBar() {
    ...
    connect(aboutAction, &QAction::triggered, [this]() {
        QMessageBox::about(this, tr("About pearOS ToDo"),
            tr("pearOS ToDo\n\n"
               "A simple to-do application for pearOS\n"
