...
class MirrorListDialog(QDialog):
    ...
    def setup_countries_section(self, layout):
        ...
        countries = [
            ...
            "Japan", "Netherlands", "Romania", "Russia", "Sweden", "United Kingdom", "United States", "Worldwide"
        ]
    ...
    def on_update_clicked(self):
        ...
        if not protocols:
            protocols = ["https"]  # Default to https if none selected
        ...
        # Build pacman-mirrors command
        command_parts = ['pkexec', 'pacman-mirrors', '-f', str(max_mirrors)]
        
        # Add countries if selected
        if self.selected_countries:
            # Map country names to pacman-mirrors format (usually lowercase, no spaces)
            country_map = {
                "Australia": "Australia",
                "Brazil": "Brazil",
                "Canada": "Canada",
                ...
                "Worldwide": None  # Worldwide doesn't need country flag
            }
            
            countries = []
            for country in self.selected_countries:
                if country != "Worldwide":
                    mapped = country_map.get(country, country.replace(" ", "_"))
                    countries.append(mapped)
            
            if countries:
                command_parts.extend(['-c'] + countries)
        
        # Add protocols
        if protocols:
            command_parts.extend(['-P'] + protocols)
        
        # Add timeout
        command_parts.extend(['-t', str(timeout)])
        
        # Build command string
        command = ' '.join(command_parts)
        
        print(f"pacman-mirrors command: {command}")
    ...
    def start_mirror_list_update(self, command):
        ...
        def run_update():
            ...
            try:
                ...
                self.signals.log_appended.emit("Starting pacman-mirrors...", "", "")
    ...
    def process_log_line(self, log_line):
        """Process a log line from pacman-mirrors"""
        self.line_counter += 1
        ...
        # pacman-mirrors output format is different from reflector
        # It typically shows mirror URLs and status messages
        # Try to parse mirror URLs
        mirror_pattern = r'^(https?://[^\s]+)'
        mirror_match = re.match(mirror_pattern, log_line)
        
        if mirror_match:
            # Found a mirror URL
            server = mirror_match.group(1)
            # Extract any additional info after the URL
            rest = log_line[len(server):].strip()
            print(f"Mirror found: {server}, rest: {rest[:50]}")
            self.signals.log_appended.emit(server, rest, "")
        elif "error" in log_line.lower() or "failed" in log_line.lower():
            # Error message
            print(f"Error detected: {log_line}")
            self.signals.log_appended.emit(log_line, "ERROR", "")
        elif "warning" in log_line.lower():
            # Warning message
            print(f"Warning detected: {log_line}")
            self.signals.log_appended.emit(log_line, "WARNING", "")
        else:
            # General log line - display as is
            print(f"General log line: {log_line[:80]}")
    ...
    def setup_blur(self):
        ...
        try:
            ...
            print("BlurWindow blur effect applied to MirrorList dialog")
        except ImportError:
            # Fallback to native KDE blur
            print("BlurWindow not available for MirrorList, using native KDE blur")
            self.setAttribute(Qt.WA_TranslucentBackground, True)
        except Exception as e:
            print(f"Error setting up blur for MirrorList: {e}")
            # Fallback to native KDE blur
            self.setAttribute(Qt.WA_TranslucentBackground, True)