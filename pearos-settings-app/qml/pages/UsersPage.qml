    ...
    SettingsCard {
        Item {
            ...
            ComboBox {
                ...
                model: ["Off"].concat(Users.users.map(u => u.username || u.fullName))
                currentIndex: Users.autoLoginUser.length > 0
                    ? Math.max(1, model.indexOf(Users.autoLoginUser))
                    : 0
                width: 120; height: 28; font.pixelSize: 12
                onActivated: {
                    if (currentIndex === 0) Users.setAutoLogin("")
                    else Users.setAutoLogin(model[currentIndex])
                }
            }
        }
    }
