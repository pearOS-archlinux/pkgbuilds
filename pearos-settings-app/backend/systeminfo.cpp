...
void SystemInfo::refresh() {
    ...
    run("findmnt -n -o SOURCE / 2>/dev/null | head -1", [this](QString out) {
        ...
        if (dev.isEmpty()) { emit infoChanged(); return; }
        // Strip btrfs subvolume bracket: "/dev/sda2[/@]" -> "/dev/sda2"
        int bracket = dev.indexOf('[');
        if (bracket != -1) dev = dev.left(bracket);
        run(QString("lsblk -no LABEL %1 2>/dev/null | head -1").arg(dev), [this](QString label) {
            QString l = label.trimmed();
            if (!l.isEmpty() && l != "Macintosh HD") {
                m_diskName = l;
            }
            // Otherwise leave m_diskName empty → QML shows "Main Storage" fallback
            emit infoChanged();
        });
    });
