...
DockManager::DockManager(QObject *parent) : QObject(parent) {
    ...
    QDir skinsDir("/usr/share/plasma/plasmoids/PearDock/contents/skins");
...
QString DockManager::readKey(const QString &key, const QString &defaultVal) const {
    ...
    for (const QString &g : m_groupPath.split('/'))
        args << "--group" << g;
    args << "--key" << key;
    QProcess proc;
...
void DockManager::writeKey(const QString &key, const QString &value) const {
    ...
    // kwriteconfig6 fails on numeric group names (e.g. containment/applet IDs).
    // Write directly to the KDE config file instead.
    QString cfgPath = QDir::homePath() + "/.config/plasma-org.kde.plasma.desktop-appletsrc";
    QFile f(cfgPath);
    if (!f.open(QIODevice::ReadOnly | QIODevice::Text)) return;

    // Target section: [Containments][2][Applets][0][Configuration][General]
    QStringList groups = m_groupPath.split('/');
    QString targetSection = "[" + groups.join("][") + "]";
