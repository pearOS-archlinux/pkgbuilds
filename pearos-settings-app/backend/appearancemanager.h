...
class AppearanceManager : public QObject {
    Q_OBJECT
    // "auto", "light", "dark" — read from themeswitcher state file
    Q_PROPERTY(QString colorScheme      READ colorScheme      NOTIFY appearanceChanged)
    // accent preset name: "blue", "purple", "orange", etc.
    Q_PROPERTY(QString accent           READ accent           NOTIFY appearanceChanged)
    ...
    Q_PROPERTY(bool tintEnabled          READ tintEnabled          NOTIFY appearanceChanged)
...
public:
    ...
    bool tintEnabled()          const { return m_tintEnabled; }
    ...
    Q_INVOKABLE void setTintEnabled(bool enabled);
...
private:
    static QString themeSwitcherScript();
    static QString themeSwitcherDir();

    QString m_colorScheme = "auto";
    QString m_accent      = "blue";
    QString m_fontSize, m_fontFamily, m_iconTheme;
