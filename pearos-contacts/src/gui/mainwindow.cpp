...
#include "../utils/contact_storage.h"
#include <QMenuBar>
#include <QMenu>
#include <QAction>
#include <QFile>
...
void MainWindow::setupUi() {
    setWindowTitle("pearOS Contacts");
...
QString iconPathForGroupName(const QString& name) {
    const QString n = name.toLower();
    if (n.contains(QStringLiteral("work")) || n.contains(QStringLiteral("job")))
    ...
    if (n.contains(QStringLiteral("shop")) || n.contains(QStringLiteral("market")) || n.contains(QStringLiteral("store")) ||
        n.contains(QStringLiteral("shopping")))
        return QStringLiteral(":/icons/sidebar_icons/shopping.svg");
    if (n.contains(QStringLiteral("family")) || n.contains(QStringLiteral("personal")))
...
void MainWindow::setupSidebar() {
    ...
    m_sidebarSearch->setPlaceholderText(tr("Filter groups"));
    ...
    QLabel* listsLabel = new QLabel(tr("My Groups"), listsHeader);
    ...
    connect(m_sideList, &QListWidget::itemDoubleClicked, this, [this](QListWidgetItem* item) {
        ...
        if (row < 0 || row >= m_contactGroups.size()) return;
        const QString currentName = m_contactGroups.at(row).name;
        bool ok = false;
        ...
        m_contactGroups[row].name = newName;
        saveContacts();
        m_currentGroupIndex = row;
    ...
    connect(m_sideList, &QListWidget::itemEntered, this, [this](QListWidgetItem* item) {
        if (!item || !m_sideList) return;
        int row = m_sideList->row(item);
        for (int i = 0; i < m_groupDeleteButtons.size(); ++i) {
            if (QPushButton* b = m_groupDeleteButtons[i]) {
                b->setVisible(i == row);
            }
        }
    });
    ...
    if (QWidget* vp = m_sideList->viewport()) {
        ...
        f->buttons = &m_groupDeleteButtons;
        f->setParent(vp);
    ...
    connect(addListButton, &QPushButton::clicked, this, [this]() {
        ContactGroup ng;
        int idx = m_contactGroups.size() + 1;
        ng.name = tr("New Group %1").arg(idx);
        ng.id = ng.name.toLower().replace(QLatin1Char(' '), QLatin1Char('-'));
        m_contactGroups.append(ng);
        m_currentGroupIndex = m_contactGroups.size() - 1;
        saveContacts();
        refreshSidebarGroups();
        if (m_currentGroupIndex >= 0 && m_currentGroupIndex < m_contactGroups.size()) {
            setCurrentGroup(m_currentGroupIndex);
        }
    });
...
void MainWindow::loadPearIdDisplayName() {
    ...
    const QStringList candidates = {
        ...
        QStringLiteral("/usr/share/pearos-contacts/pearID/get_user_info.sh")
    };
...
void MainWindow::setupContent() {
    ...
    m_contactWidget = new ContactWidget(this);
    m_contentLayout->addWidget(m_contactWidget, 1);

    loadContacts();
    refreshSidebarGroups();

    connect(m_contactWidget, &ContactWidget::contactAdded, this, [this](const Contact& contact) {
        if (m_currentGroupIndex < 0 || m_currentGroupIndex >= m_contactGroups.size()) return;
        m_contactGroups[m_currentGroupIndex].contacts.append(contact);
        saveContacts();
        setCurrentGroup(m_currentGroupIndex);
    });

    connect(m_contactWidget, &ContactWidget::contactUpdated, this, [this](int index, const Contact& contact) {
        if (m_currentGroupIndex < 0 || m_currentGroupIndex >= m_contactGroups.size()) return;
        QVector<Contact>& contacts = m_contactGroups[m_currentGroupIndex].contacts;
    ...
    connect(m_contactWidget, &ContactWidget::contactDeleted, this, [this](int index) {
        if (m_currentGroupIndex < 0 || m_currentGroupIndex >= m_contactGroups.size()) return;
        QVector<Contact>& contacts = m_contactGroups[m_currentGroupIndex].contacts;
        if (index < 0 || index >= contacts.size()) return;
    ...
    if (!m_contactGroups.isEmpty()) {
        setCurrentGroup(0);
    }
}

void MainWindow::loadContacts() {
    m_contactGroups = ContactStorage::load();
    if (m_contactGroups.isEmpty()) {
        m_currentGroupIndex = -1;
    } else if (m_currentGroupIndex < 0 || m_currentGroupIndex >= m_contactGroups.size()) {
        m_currentGroupIndex = 0;
    }
}

void MainWindow::saveContacts() const {
    if (m_contactGroups.isEmpty()) return;
    if (!ContactStorage::save(m_contactGroups)) {
        Logger::warning(QStringLiteral("Failed to save contacts"));
    }
}

void MainWindow::refreshSidebarGroups() {
    if (!m_sideList) return;
    QSignalBlocker blocker(m_sideList);
    m_sideList->clear();
    m_groupDeleteButtons.clear();

    const QPalette pal = qApp->palette();
    const QColor accent = pal.color(QPalette::Highlight);
    const QColor text = QColor(QStringLiteral("#fafafa"));

    for (int i = 0; i < m_contactGroups.size(); ++i) {
        const ContactGroup& group = m_contactGroups.at(i);
        ...
        const bool selected = (i == m_currentGroupIndex);

        // Icon
        auto* iconLabel = new QLabel(row);
        iconLabel->setFixedSize(20, 20);
        iconLabel->setAlignment(Qt::AlignCenter);
        const QString iconPath = iconPathForGroupName(group.name);
        ...
        // Text
        auto* label = new QLabel(group.name, row);
        ...
        m_groupDeleteButtons.append(deleteBtn);

        connect(deleteBtn, &QPushButton::clicked, this, [this, i]() {
            if (i < 0 || i >= m_contactGroups.size()) return;
            m_contactGroups.removeAt(i);
            saveContacts();
            if (m_contactGroups.isEmpty()) {
                m_currentGroupIndex = -1;
                if (m_contactWidget) {
                    ContactGroup empty;
                    empty.name = tr("Empty");
                    m_contactWidget->setGroup(empty);
                }
            } else if (m_currentGroupIndex >= m_contactGroups.size()) {
                m_currentGroupIndex = m_contactGroups.size() - 1;
            }
            refreshSidebarGroups();
            if (m_currentGroupIndex >= 0 && m_currentGroupIndex < m_contactGroups.size()) {
    ...
    if (!m_contactGroups.isEmpty()) {
        const int idx = (m_currentGroupIndex >= 0 && m_currentGroupIndex < m_contactGroups.size())
                        ? m_currentGroupIndex
                        : 0;
...
void MainWindow::setCurrentGroup(int index) {
    if (index < 0 || index >= m_contactGroups.size()) {
        m_currentGroupIndex = -1;
        return;
    }
    m_currentGroupIndex = index;
    if (m_contactWidget) {
        m_contactWidget->setGroup(m_contactGroups.at(index));
    }
    refreshSidebarGroups();
...
void MainWindow::createMenuBar() {
    ...
    connect(aboutAction, &QAction::triggered, [this]() {
        QMessageBox::about(this, tr("About pearOS Contacts"),
            tr("pearOS Contacts\n\n"
               "A simple contacts application for pearOS\n"
...
void MainWindow::loadStyleSheet() {
    ...
    if (!styleFile.exists()) styleFile.setFileName("/usr/share/pearos-contacts/stylesheet.qss");
